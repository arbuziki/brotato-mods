...
func get_spawn_pos_in_area(base_pos:Vector2, area:int, spawn_dist_from_edges:int = 0, spawn_edge_of_map:bool = false)->Vector2:
	...
	if RunData.current_wave > 10:
		x = (RunData.current_wave - 10.0) * (center.x - MIN_DIST_FROM_PLAYER - Utils.EDGE_MAP_DIST) / 20.0
		y = (RunData.current_wave - 10.0) * (center.y - MIN_DIST_FROM_PLAYER - Utils.EDGE_MAP_DIST) / 20.0
